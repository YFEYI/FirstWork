...
class CNN_Block(nn.Module):
    def __init__(self,in_C=10,out_C=72,ratio=2,kernel_list=(1,3,5,7),drop=0.):
        ...
        #self.conv_after_band=cbam(in_size=out_C,reduction_ratio=ratio,pool_types=['avg','max'],kernel_size=3)
    def forward(self,x):
...
def CCCA_small(num_classes: int = 4, **kwargs):
...
def CCCA_base(num_classes: int = 4, **kwargs):
...
def CCCA_large(num_classes: int = 4, **kwargs):
