...
from CCCC.CCCA import CCCA_small,CCCA_base,CCCA_large
...
def main(model,data,label,type='small'):
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1)%100==0:
            np.save("./CCCA_A_{}.npy".format(type),np_log)
    checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,
                  'lr': scheduler.state_dict()}
    torch.save(checkpoint, "./CCCA_A_{}.pth".format(type))
...
if __name__ == '__main__':
    ...
    model = CCCA_small(num_classes=4).to(device)
