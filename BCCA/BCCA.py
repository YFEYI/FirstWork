...
class CNN_Block(nn.Module):
    def __init__(self,in_C=10,out_C=72,ratio=2,kernel_list=(1,3,5,7),drop=0.):
        ...
        self.cbam_after_fusion = cbam(in_size=out_C, reduction_ratio=ratio, pool_types=['avg', 'max'], kernel_size=3)
...
class CCModule(nn.Module):
    def __init__(self, in_chans=10,num_classes=2,embed_dim=72,batch_size=32, depths=None,
        ...
        self.conv_span = nn.Sequential(
            nn.Conv2d(in_channels=in_chans, out_channels=embed_dim, kernel_size=(1,1), stride=1, padding=0),
            nn.ReLU(),
            nn.Conv2d(in_channels=embed_dim, out_channels=embed_dim, kernel_size=(1,1), stride=1, padding=0),
            nn.ReLU()
        )   #并行组网时候使用的扩张卷积
        ...
        self.last_mlp=Mlp(in_features=self.embed_dim*81,hidden_features=int(self.embed_dim*81/2),out_features=1024,act_layer=nn.ReLU,drop=0.2)
...
def BCCA_small(num_classes: int = 2, **kwargs):
...
def BCCA_base(num_classes: int = 2, **kwargs):
    model = CCModule(in_chans=5,num_classes=num_classes,embed_dim=72,batch_size=64,depths=[3],num_heads=[2],patch_size=3,mlp_ratio=4,qkv_bias=True
                     ,drop=0.,attn_drop=0.,ape=True)
    return model

def BCCA_large(num_classes: int = 2, **kwargs):
    model = CCModule(in_chans=5,num_classes=num_classes,embed_dim=144,batch_size=64,depths=[3],num_heads=[2],patch_size=3,mlp_ratio=4,qkv_bias=True
                     ,drop=0.,attn_drop=0.,ape=True)
    return model
