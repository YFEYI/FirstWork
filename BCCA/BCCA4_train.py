...
from BCCA.BCCA4 import BCCA4_small,BCCA4_base,BCCA4_large
...
def train_one_epoch(model,optimizer,data_loader,device,epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        labels=F.one_hot(labels,4).float()
...
def main(model,data,label,type='small'):
    rand=random.seed(0)
    batch_size=64
    epochs=300
    lr=0.0001
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1)%100==0:
            np.save("./BCCA4_{}.npy".format(type),np_log)
    checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,
                  'lr': scheduler.state_dict()}
    torch.save(checkpoint, "./BCCA4_{}.pth".format(type))
...
if __name__ == '__main__':
    ...
    model = BCCA4_base(num_classes=4).to(device)
    main(model=model,data=data,label=arousal_valence,type='small')