...
class CCModule(nn.Module):
    def __init__(self, in_chans=10,num_classes=2,embed_dim=144,batch_size=32, depths=None,
        ...
        #ccattn_CAV_ver0.10   #C->72?
        #self.cnn_block=CNN_Block(in_C=in_chans,out_C=embed_dim,ratio=mlp_ratio,kernel_list=(1,3,5,7))

        # ccattn_MAV_ver0.10  #C->72?
        # self.inrlayer1 = InvertedResidual(in_channels=in_chans,out_channels=embed_dim, stride=1, hidden_dim=embed_dim,groups_head=1,use_res_connect=False)
        # self.inrlayer2 = InvertedResidual(in_channels=embed_dim , out_channels=embed_dim, stride=1, hidden_dim=embed_dim,groups_head=1, use_res_connect=True)
        # self.inrlayer3 = InvertedResidual(in_channels=embed_dim, out_channels=embed_dim, stride=1, hidden_dim=int(embed_dim*2),groups_head=1, use_res_connect=True)
        # self.inrlayer4 = InvertedResidual(in_channels=embed_dim, out_channels=embed_dim, stride=1, hidden_dim=int(embed_dim*2),groups_head=1, use_res_connect=True)

        # ccattn_RAV_ver0.10   #C->144?
        self.resnet34=resnet34(num_classes=2,include_top=False)
        ...
        self.fusion=nn.Sequential(
            nn.Conv2d(in_channels=embed_dim*2,out_channels=embed_dim,kernel_size=(3,3),stride=1,padding=1),
            nn.ReLU(),
        )

        #self.last_mlp=Mlp(in_features=self.embed_dim*81,hidden_features=1024,out_features=512,act_layer=nn.ReLU,drop=0.2)

        self.last_mlp1 = Mlp(in_features=self.embed_dim * 81, hidden_features=2560, out_features=1024, act_layer=nn.ReLU,
                            drop=0.2)
    ...
    def forward(self,x):
        # x: [B, C, H, W]
        B, C, H, W = x.size()
        #ver1.0 CNN+ATTN Arousal和 Value两个维度二分类
        #x = self.cnn_block(x)

        # ver1.1 MobileNet+ATTN Arousal和 Value两个维度二分类
        # x=self.inrlayer1(x)
        # x=self.inrlayer2(x)
        # x=self.inrlayer3(x)
        # x = self.inrlayer4(x)

        # ver1.1 ResNet+ATTN Arousal和 Value两个维度二分类
        x=self.resnet34(x)
...
def resattn(num_classes: int = 2, **kwargs):
    model = CCModule(in_chans=5,num_classes=num_classes,embed_dim=144,batch_size=32,depths=[1],num_heads=[2],patch_size=3,mlp_ratio=4,qkv_bias=True
