...
def main(model,data,label):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            np.save("./resnet_attn_V.npy".format(type), np_log)
            np.save('./resnet_attn_TFPN_V.npy'.format(type), dict)
    checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,
                  'lr': scheduler.state_dict()}
    torch.save(checkpoint, "./resnet_attn_V.pth".format(type))
...
if __name__ == '__main__':
    ...
    model = resattn(num_classes=2).to(device)
