...
from CC.mobile_attn import mobile_attn
"""
运行方法
"""
...
def main(model,data,label):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            np.save("./mobile_attn_V .npy", np_log)
            np.save('./mobile_attn_TFPN_V.npy', dict)
    checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,
                  'lr': scheduler.state_dict()}
    torch.save(checkpoint, "./mobile_attn_V.pth")
...
if __name__ == '__main__':
    ...
    model = mobile_attn(num_classes=2).to(device)
    main(model=model,data=data,label=valence)