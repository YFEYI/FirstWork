...
class ConvLayer(nn.Module):
    def __init__(self,in_channels: int,out_channels: int,kernel_size: int,stride: int = 1,groups_head: int = 1,
        bias: bool= False,use_norm: bool= True,use_act: bool= True):
...
class InvertedResidual(nn.Module):
    def __init__(self,in_channels:int, out_channels:int, stride:int, hidden_dim: int,
                 groups_head:int=1,use_res_connect:bool= True):
        ...
        block.add_module(
            ...
            module=ConvLayer(
                ...
                stride=stride,
                kernel_size=3,
                groups_head=int(hidden_dim/groups_head)
            ),
        )
        block.add_module(
            name="red_1x1",
...
def mobile_attn(num_classes: int = 2, **kwargs):
    model = CCModule(in_chans=5,num_classes=num_classes,embed_dim=72,batch_size=32,depths=[1],num_heads=[2],patch_size=3,mlp_ratio=4,qkv_bias=True
